...
// Example Usage
(async () => {
    const passwordA = "userA_password";
    const passwordB = "userB_password";
    const usernameA = "userA";
    const usernameB = "userB";

    // Generate keys for User A
    const userAKeys = await generateKeypair(passwordA, usernameA);
    console.log("User A Private Key:", userAKeys.privateKey);
    console.log("User A Public Key:", userAKeys.publicKey);

    // Generate keys for User B
    const userBKeys = await generateKeypair(passwordB, usernameB);
    console.log("User B Private Key:", userBKeys.privateKey);
    console.log("User B Public Key:", userBKeys.publicKey);

    // Both users derive the same shared key
    const sharedKeyA = await deriveSharedKey(userAKeys.privateKey, userBKeys.publicKey);
    const sharedKeyB = await deriveSharedKey(userBKeys.privateKey, userAKeys.publicKey);

    console.log("Shared Key A:", sodium.to_hex(sharedKeyA));
    console.log("Shared Key B:", sodium.to_hex(sharedKeyB));

    console.assert(
        sodium.to_hex(sharedKeyA) === sodium.to_hex(sharedKeyB),
