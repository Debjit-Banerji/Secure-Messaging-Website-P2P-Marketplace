...
import { deriveAESKey, encryptData, decryptData } from "../utils/cryptoUtils"; // Import AES-ECB functions

export default function ProfilePage() {
  const dispatch = useDispatch();
  const [profilePicture, setProfilePicture] = useState("");
  const curUserEncrypted = useSelector((state) => state.user.user); // Encrypted user data
  const password = useSelector((state) => state.user.password);
  const [curUser, setCurUser] = useState(null); // Decrypted user data
  ...
  useEffect(() => {
    if (curUserEncrypted) {
      const decryptUserData = async () => {
        try {
          const key = await deriveAESKey(password); // Use a secure key
          const decryptedEmail = await decryptData(key, curUserEncrypted.email_cipher.ciphertext, curUserEncrypted.email_cipher.iv);
          setTempUser(curUserEncrypted);
          setTempUser((prev) => ({
            ...prev,
            email: decryptedEmail,
          }))
          if(curUserEncrypted.phone_cipher) {
            const decryptedPhone = await decryptData(key, curUserEncrypted.phone_cipher.ciphertext, curUserEncrypted.phone_cipher.iv);
            setTempUser((prev) => ({
             ...prev,
  ...
  const handleSaveChanges = async (field) => {
    try {
      ...
      if(field === "email"){
        const encryptedEmail = await encryptData(key, tempUser.email)
        dispatch(updateProfile({"email_cipher": encryptedEmail})); // Store encrypted data in Redux
      }
      else if(field === "phone"){
        const encryptedPhone = await encryptData(key, tempUser.phone)
        dispatch(updateProfile({"phone_cipher": encryptedPhone})); // Store encrypted data in Redux
      }
      else{
