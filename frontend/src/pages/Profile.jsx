...
import { fetchProductsByUser, createProduct, deleteProduct, editProduct } from "../store/slices/product-slice";
import UserProductCard from "../components/UserProductCard.jsx";
import reducer from "../store/slices/chat-slice.js";
...
export default function ProfilePage() {
  ...
  const handleProfilePictureChange = (e) => {
    ...
    if (file) {
      ...
      reader.addEventListener('load', () => {
        const base64String = reader.result // Remove Base64 prefix
  ...
  const handleOTPSubmit = () => {
    dispatch(verifyOTP({ email: userData.email, otp: otp }))
    .unwrap()
    .then(() => {
      setShowOTPOverlay(false);
      setOTP("");
      setIsEditable[fieldToBeUpdated] = false;
      dispatch(updateProfile({ [fieldToBeUpdated]: tempUser[fieldToBeUpdated], [fieldToBeUpdated+"_verified"]: true }));
    })
    .catch(error => {
  ...
  const handleNewProductImage = (e) => {
    ...
    if (file) {
      ...
      reader.addEventListener("load", () => {
        ...
        setNewProduct((prev) => ({
          ...prev,
          image: base64String, // ✅ Store actual File object
        }));
      });
      reader.readAsDataURL(file);
    }
  ...
  const handleAddProduct = () => {
    ...
    const updated = {
      ...
      price: newProduct.price.replace("Rs.", ""),
      description: newProduct.description,
      image: newProduct.image,
      user: userData.id,
    };

    dispatch(createProduct({...updated})); // ✅ Pass FormData instead of raw object
  ...
  const handleEdit = () => {
    dispatch(editProduct({ productId: editedProduct.id, productData: editedProduct }));
  ...
  const handleProductImage = (e) => {
    if (e.target.files && e.target.files[0]) {
      const reader = new FileReader();
      reader.onload = (event) => {
        setEditedProduct({ ...editedProduct, image: event.target.result });
      };
    ...
    <PageContainer>
      <ProfileContainer>
        ...
        {activeTab === "profile" ? (
          ...
          (userData?.email_verified || userData?.phone_verified) && <ProductsContent>
            <ProductsHeader>
              <div>
            ...
            {userProducts && userProducts.length === 0 ? (
              ...
              <ProductGrid>
                {userProducts.map(product => (
                  <UserProductCard key={product.id} product={product} onDelete={handleDeleteProduct} onEdit={() => handleEditClick(product)} />
                ))}
              </ProductGrid>
            )}
          </ProductsContent>
      ...
      {(userData?.email_verified || userData?.phone_verified) && isEditingProduct && (
        <ModalOverlay onClick={() => setIsEditingProduct(false)}>
          <ModalContent onClick={(e) => e.stopPropagation()} wider>
            <ModalHeader>
              <h3>Edit Product</h3>
              <FaTimes onClick={() => setIsEditingProduct(false)} />
            </ModalHeader>
            <ModalBody>
              <ProductFormGrid>
              ...
              <ModalActions>
                <CancelButton onClick={() => setIsEditingProduct(false)}>
                  Cancel
                </CancelButton>
      ...
      {showOTPOverlay && (
        <ModalOverlay onClick={() => setShowOTPOverlay(false)}>
          <ModalContent onClick={(e) => e.stopPropagation()}>
            ...
            <ModalBody>
              <p>We've sent a 6-digit code to your emailID</p>
      ...
      {/* Add Product Modal */}
      {(userData?.email_verified || userData?.phone_verified) && showAddProductModal && (
